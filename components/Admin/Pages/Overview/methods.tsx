export const userFilter = (filter, x, y) => {
  if (filter === 'Sort by Pending') {
    if (x.uid && y.uid) {
      if (x.qualified === '') {
        if (y.qualified === 'yes') { return -1 }
        else if (y.qualified === 'no') { return -1 }
        else if (y.qualified === '') { return 0 }
      }
      else if (x.qualified === 'yes') {
        if (y.qualified === 'yes') { return 0 }
        else if (y.qualified === 'no') { return -1 }
        else if (y.qualified === '') { return 1 }
      }
      else if (x.qualified === 'no') {
        if (y.qualified === 'yes') { return 1 }
        else if (y.qualified === 'no') { return 0 }
        else if (y.qualified === '') { return 1 }
      }
    }
...
export const userViewDisplay = (view, user) => {
  if (view === 'All Users') {
    ...
    return (user.qualified === 'yes')
  }
  else if (view === 'Rejected') {
    return (user.qualified === 'no')
  }
  else if (view === 'Checked-In') {
    return user.qualified === 'yes' && user.checkedIn
  }
